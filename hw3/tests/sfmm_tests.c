...
//############################################
Test(sf_memsuite_student, free_without_coalesce, .init = sf_mem_init, .fini = sf_mem_fini) {
	sf_errno = 0;
	sf_malloc(38);
    void *b = sf_malloc(96);
    sf_malloc(287);
	...
	assert_free_block_count(2);
	assert_free_list_count(112, 1);
	assert_free_list_count(3456, 1);
	cr_assert(sf_errno == 0, "sf_errno is not zero!");
}


Test(sf_memsuite_student, free_coalesce_with_next, .init = sf_mem_init, .fini = sf_mem_fini) {
	sf_errno = 0;
	void *a = sf_malloc(38);
    void *b = sf_malloc(96);
    sf_malloc(287);
...
Test(sf_memsuite_student, free_coalesce_with_prev, .init = sf_mem_init, .fini = sf_mem_fini) {
	sf_errno = 0;
	void *a = sf_malloc(38);
    void *b = sf_malloc(96);
    sf_malloc(287);
...
Test(sf_memsuite_student, free_coalesce_with_prev_and_next, .init = sf_mem_init, .fini = sf_mem_fini) {
	sf_errno = 0;
	void *a = sf_malloc(38);
    void *b = sf_malloc(96);
    void *c = sf_malloc(287);
    sf_malloc(115);
	...
	assert_free_block_count(2);
	assert_free_list_count(464, 1);
	assert_free_list_count(3456, 1);
	cr_assert(sf_errno == 0, "sf_errno is not zero!");
}

Test(sf_memsuite_student, malloc_with_new_page_and_free_all, .init = sf_mem_init, .fini = sf_mem_fini) {
	sf_errno = 0;
	void *a = sf_malloc(PAGE_SZ+2000);
    
    sf_free(a);

	assert_free_block_count(1);
	assert_free_list_count(8144, 1);
	cr_assert(sf_errno == 0, "sf_errno is not zero!");
}
